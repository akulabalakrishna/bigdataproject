...
| **2. MinIO lakehouse storage** | `bronze`, `silver`, and `gold` zones mapped locally to MinIO S3-compatible service. | `docker-compose.yml` (MinIO service) | ⚠️ Partial Risk | Local storage utilizes strict **Parquet** format instead of full ACID Delta/Iceberg layers. |
...
| **6. ML model trained from lakehouse** | Algorithm extracts Gold parquet layer to train pipeline for prediction. | `src/training/train.py` | ⚠️ Partial Risk | Pipeline uses localized **scikit-learn** (`LogisticRegression`) rather than distributed Spark MLlib or Ray. |
